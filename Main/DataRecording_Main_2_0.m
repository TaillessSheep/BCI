...
function DataRecording_Main_2_0()
clear;clc;
...
img(1).file = imread('C2_LH.png');  
img(2).file = imread('C2_RH.png');  
...
% img(1).file = imread('C4_LH.png');  
% img(2).file = imread('C4_RH.png');  
% img(3).file = imread('C4_LF.png');  
% img(4).file = imread('C4_RF.png');    
% img(5).file = imread('C.png');      
...
%     data_received = single(zeros(totalSamples,35));
    ...
    for current_trial = (1:trialNum)
        ...
        % generate break time 
        BreakTime = breakDuration(1) + rand * (breakDuration(2)-breakDuration(1));
        disp(BreakTime)
        % recording
        while toc <= BreakTime
            % read data
