...
ExCode={[0,0,0,2,2,2],[1,2,2,0,0,2],[2,1,2,1,2,0],[2,2,1,2,1,1]};
...
val = zeros(length(temp),size(trialLabel,2)); Ny = 10;
for j=1:length(temp)
...
%             [~, indy] = sort(abs(y),'descend');
        ...
        res(res == -1) = 1;
        res(isnan(res)) = 0;
        select = [1,2 ; 1,3 ; 1,4 ; 2,3 ; 2,4 ; 3,4];
        for kkk = 1:6
            sel(kkk) = select(kkk,res(kkk)+1);
        end
        Result(j,1) = mode(sel);
...
save(['OVO-' datestr(now, 'yyyy-mmm-dd-HH-MM') '.mat'], 'final')
