...
% 
clc; clear; close all; 
cd ('C:\\Users\\Tim\\ownCloud\\Electrical_Engineering\\summer_2017_project');
...
Tcut = 0.6;         %amount to trim for filter artefacts
Tdelay = 0;       %delay between button press and recording
...
Ntr_min = 10;                %number of training epocs before first model
score_threshold = 0.7;
Passband = [8 30];
...
if robot_on
%     robot = legoev3('usb');
%     robot = legoev3('Bluetooth','COM9');
    robot = legoev3('bt','00165353713E');
    clearLCD(robot);
...
%         end
    ...
    if readDigitalPin(ard,pin_stop) == 1
        ...
        motor_l.Speed = 0;
        pause(2);
        while readDigitalPin(ard,pin_stop) == 0
        end
%         running =0;
    end
end
