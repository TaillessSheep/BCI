...
Ntr_min = 8;                %number of before first model
score_threshold = 0.7;
Passband = [0.5 35];
...
while running == 1
    ...
    if State_tr == 1 && toc(time_tr)>(Tepoc+Tdelay)
        ...
        if length(Label_tr)>=Ntr_min && length(unique(Label_tr))==Ncl
            ...
            [Wn, f_tr, Class_comb, Ncomb]=TrainCSP02( Data_tr, Label_tr, 2 );
            % Train classifier
            Mdl=cell(1,Ncomb);
            for comb = 1:Ncomb
...
%     % display classification output
    if ~isempty(Mdl)
        ...
        for comb = 1:Ncomb
            arg_ts  = Wn(:,:,comb)'* EEG_window_filt(:,:) * EEG_window_filt(:,:)'* Wn(:,:,comb);
            f_ts = log ((diag(arg_ts))/trace(arg_ts));
            [~, Out_scores(comb,:)] = predict(Mdl{1,comb},f_ts');
        end
        % display spectrum
        if display_spec
            EEG_window_spafilt=Wn'*EEG_window_filt;
            figure(F_spec);
        ...
        if display_feature
            ...
            for i = 1:Ntr
                if Label_tr(1,i) == 1
                    plot(f_tr(i,1,1), f_tr(i,4,1), 'b.');
                elseif Label_tr(1,i) == 2
                    plot(f_tr(i,1,1), f_tr(i,4,1), 'r.');
                elseif Label_tr(1,i) == 3
                    plot(f_tr(i,1,1), f_tr(i,4,1), 'c.');
                elseif Label_tr(1,i) == 4
                    plot(f_tr(i,1,1), f_tr(i,4,1), 'm.');
                elseif Label_tr(1,i) == 5
                    plot(f_tr(i,1,1), f_tr(i,4,1), 'y.');
                end
                hold on
            end
...
%         end
        if robot_on  && State_tr==0
            motor_l.Speed = -topspeed*2*(Out_score(1,1)-0.5);
            motor_r.Speed = -topspeed*2*(Out_score(1,2)-0.5);
